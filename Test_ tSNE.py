...
parser.add_argument(
    "--models-dir",
    type=str,
    default="./models/260209_105821",  # 260206_084839 or 260209_105821
...
def add_vehicle_points(df_data, label, per_vehicle=200, rng=None):
    rng = np.random.default_rng(0) if rng is None else rng
    Xv = np.asarray(df_data, dtype=float)
    n = Xv.shape[0]
    if n == 0:
        return None, None
    idx = rng.choice(n, size=min(per_vehicle, n), replace=False)
...
test_list = [normal_list[0:cnt_max], [347]]
...
for label, vehicle_ids in enumerate(test_list):
    for i in vehicle_ids:
        ...
        _, t2_contrib = T2_array(df_data, data_mean, data_std, p_k, v_I)
        _, spe_contrib = SPE_array(df_data, data_mean, data_std, p_k)
