...
for i in validate_list:
    ...
    v_tensor = safe_load(f'{args.source_data_dir}/{BATTERY_TYPE}/{i}/vin_2.pkl', verbose=False)
    v_tensorx = safe_load(f'{args.source_data_dir}/{BATTERY_TYPE}/{i}/vin_3.pkl', verbose=False)
    v_tensor, v_tensorx = preprocess_combined_tensor(v_tensor, v_tensorx, dim_dict, 
                                                BATTERY_TYPE, PREPROCESSING, SKIP_CHARGE_READY, 
                                                normalize_dx=args.normalize_dx, normalize_val=args.normalize_val)
    if FIRST_LOAD:
        FIRST_LOAD = False
...
v_y_recovered = v_combined_tensor[:, dim_dict["x"]:dim_dict["x"] + dim_dict["y"]] # 2~111
v_z_recovered = v_combined_tensor[:, dim_dict["x"] + dim_dict["y"]: dim_dict["x"] + dim_dict["y"] + dim_dict["z"]] # 112~221
v_q_recovered = v_combined_tensor[:, dim_dict["x"] + dim_dict["y"] + dim_dict["z"]:] # 222~

v_x_recovered2 = v_combined_tensorx[:, :dim_dict["x2"]]
v_y_recovered2 = v_combined_tensorx[:, dim_dict["x2"]:dim_dict["x2"] + dim_dict["y2"]]
v_z_recovered2 = v_combined_tensorx[:, dim_dict["x2"] + dim_dict["y2"]: dim_dict["x2"] + dim_dict["y2"] + dim_dict["z2"]]
v_q_recovered2 = v_combined_tensorx[:, dim_dict["x2"] + dim_dict["y2"] + dim_dict["z2"]:]
...
for epoch in range(AE_U_EPOCH):
    ...
    if epoch % 10 == 0:
        ...
        print('Epoch: {:2d} | Train Loss: {:.4f} | Val Loss: {:.4f}'.format(epoch, avg_loss, avg_val_loss))
        # checkpoint (indexed)
        save_net_state(model=net, models_dir=f"{model_path}/", filename=f"net_e{epoch}.pth")

# Save loss log (fast: dump once per model)
np.savetxt(
    os.path.join(f"{data_path}/", "loss_net.csv"),
    np.column_stack((np.arange(len(avg_loss_list_u), dtype=np.int64), np.asarray(avg_loss_list_u, dtype=np.float64))),
    delimiter=",",
    fmt=["%.0f", "%.4f"],
...
for epoch in range(AE_X_EPOCH):
    ...
    if epoch % 10 == 0:
        ...
        print('Epoch: {:2d} | Train Loss: {:.4f} | Val Loss: {:.4f}'.format(epoch, avg_loss, avg_val_loss))
        # checkpoint (indexed)
        save_net_state(model=netx, models_dir=f"{model_path}/", filename=f"netx_e{epoch}.pth")

# Save loss log (fast: dump once per model)
np.savetxt(
    os.path.join(f"{data_path}/", "loss_netx.csv"),
    np.column_stack((np.arange(len(avg_loss_list_x), dtype=np.int64), np.asarray(avg_loss_list_x, dtype=np.float64))),
    delimiter=",",
    fmt=["%.0f", "%.4f"],
    header="epoch,avg_loss",
    comments="",
